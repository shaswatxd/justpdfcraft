...
export const ToolExplorer: React.FC<ToolExplorerProps> = ({
  ...
  return (
    <section id="tool-explorer" className="w-full max-w-5xl mx-auto px-4 space-y-6">
      {/* Search & Category Filter Header Bar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-slate-900/80 p-3 rounded-2xl border border-slate-800 shadow-md backdrop-blur-md">
        {/* Category Tabs */}
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none">
          {[
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all select-none ${
                  isActive
                    ? 'bg-swift-600 text-white shadow-md shadow-swift-900/30'
          ...
          >
            <Heart className={`w-3.5 h-3.5 ${showOnlyFavorites ? 'fill-current' : 'text-rose-400'}`} />
        ...
        <div className="relative flex-1 md:max-w-xs">
          ...
          <input
            type="text"
            placeholder="Search all 45+ tools... (e.g. compress, cgpa)"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-slate-950/80 border border-slate-700 rounded-xl pl-9 pr-8 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-swift-500 focus:ring-1 focus:ring-swift-500 transition-all font-sans"
          />
          {searchQuery && (
            <button
      ...
      {/* Recently Used Tray (When visible and no active search query) */}
      ...
      <div className="space-y-3">
        ...
        {filteredTools.length === 0 ? (
          <div className="py-12 px-4 rounded-2xl bg-slate-800/30 border border-slate-800 text-center space-y-3">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              {showOnlyFavorites
                ...
                : `We couldn't find anything matching "${searchQuery}". Try searching for compress, merge, cgpa, or ocr.`}
            </p>
            {(searchQuery || showOnlyFavorites) && (
              <button
                ...
                className="px-4 py-2 bg-swift-600 hover:bg-swift-500 text-white rounded-xl text-xs font-semibold transition-colors"
              >
                Show All Tools
              </button>
            )}
